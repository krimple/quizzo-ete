...
public class GameStatusController extends AbstractQuizController {
  ...
  GameStatus getGameStatus(HttpSession session) {
    ...
    if (!playerGameSession.hasPlayerAndGame()) {
      return new NoGameSessionCreated();
    }

    // another case - do we actually have a game?
    if (!playerGameSession.hasGame()) {
      return new NoGameSessionCreated();
    }

    if (!gameRunEngine.gameExists(playerGameSession.getGameId())) {
    ...
    try {
      ...
      // kludge for now, FIX
      // means we can't find a game that matches...
      return new GameNotFound(playerGameSession.getGameId());
    }
